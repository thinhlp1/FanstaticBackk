...
public class WarehouseReceiveItemService {
    ...
    public ResponseDTO create(WarehouseReceiveItemRequestDTO warehouseReceiveItemRequestDTO) {
        /*
         * Tạo 1 đối tượng supplier dựa trên supplierId được truyền vào
         * warehouseReceiveRequestDTO
         */
        Flavor flavor = flavorRepository.findByIdAndActiveIsTrue(warehouseReceiveItemRequestDTO.getFlavorId())
                .orElse(null);
        WarehouseReceive warehouseReceive = warehouseReceiveRepository
                .findByIdAndActiveIsTrue(warehouseReceiveItemRequestDTO.getWarehouseReceiveItemId()).orElse(null);

        List<FieldError> errors = new ArrayList<>();
        // Bắt lỗi danh mục không tồn tại -> cái này không câần in ra cho user, dev coi
        // được rồi
        if (flavor == null) {
         ...
         * WarehouseReceive warehouseReceiveSaved =
         * warehouseReceiveRepository.save(warehouseReceive);
         ...
         * 
         * if (warehouseReceiveSaved != null) {
         * systemService.writeSystemLog(warehouseReceiveSaved.getId(),
         * warehouseReceiveSaved.getDescription(), null);
         * //Mã 200 là thêm thành công (Khi success thì không truyền data nào để return
         * cho json cả mà chỉ thông báo thành công)
         * return ResponseUtils.success(200, MessageConst.UPDATE_SUCCESS, null);
         * }
         */

        WarehouseReceiveItem warehouseReceiveItem = modelMapper.map(warehouseReceiveItemRequestDTO,
                WarehouseReceiveItem.class);
        ...
        // save and flush dung cho việc save vào database và lấy dữ liệu lên ngay lập
        // tức để xử lý
        WarehouseReceiveItem warehouseReceiveItemSaved = warehouseReceiveItemRepository
                .saveAndFlush(warehouseReceiveItem);

        if (warehouseReceiveItemSaved != null) {
            // Lưu thông tin vào lịch sử hệ thống
            // systemService.writeSystemLog(warehouseReceiveItemSaved.getId(),
            // warehouseReceiveItemSaved.getWarehouseReceive().getId(), null);
            /*
             * //Mã 200 là thêm thành công (Khi success thì không truyền data nào để return
             * cho json cả mà chỉ thông báo thành công)
             */
    ...
    public ResponseDTO show() {// truyền tham số active vào để show list voucher dựa vào trạng thái
        ...
        // Sắp xếp danh sách theo createAt giảm dần
        warehouseReceiveItems.sort(Comparator.comparing(WarehouseReceiveItem::getCreateAt).reversed());
    ...
    public ResponseDTO showByFlavorId(int flavorId) {// truyền tham số active vào để show list voucher dựa vào trạng
                                                     // thái
        // Tạo 1 list voucher mới chưa có dữ liệu
        List<WarehouseReceiveItem> warehouseReceiveItems = warehouseReceiveItemRepository.findAllByFlavorId(flavorId)
                .orElse(null);
