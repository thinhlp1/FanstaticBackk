...
public class FlavorService {
    ...
    public ResponseDTO create(FlavorRequestDTO flavorRequestDTO) {
        ...
        Category category = categoryRepository.findByIdAndActiveIsTrue(flavorRequestDTO.getCategoryId()).orElse(null);
//        File fileImage = fileRepository.findByIdAndActiveIsTrue(extraPortionRequestDTO.getImageId()).orNull();
        ...
        if (category == null) {
            errors.add(new FieldError("flavorRequestDTO", "categoryId", "Danh mục không tồn tại"));
        }
    ...
    public ResponseDTO detail(int id) {
        ...
        if (flavor.getCategory() != null) {
            CategoryDTO categoryDto = modelMapper.map(flavor.getCategory(), CategoryDTO.class);
            flavorDTO.setCategoryDTO(categoryDto);
        }
